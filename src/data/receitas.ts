export const receitas = [
    // === Receita 1: Bolo de Frutas Vermelhas ===
    {
        "id": 1,
        "nome": "Bolo de Frutas Vermelhas",
        "categoria": "Sobremesa",
        "tags": [
            "#bolo",
            "#doce",
            "#frutas",
            "#confeitaria"
        ],
        "forma": "Bolo inglês 30x10cm",
        "rendimento": "aprox. 10 a 12 fatias ",
        "tempo_preparo": {
            "preparo": "30 minutos",
            "forno": "40 a 45 minutos",
            "total": "1h10min a 1h15min"
        },
        "ingredientes": {
            "Massa": [
                "4 ovos",
                "1 xícara de açúcar (200g)",
                "1 colher de chá de essência de baunilha",
                "60ml de óleo",
                "70ml de leite",
                "1 xícara de farinha de trigo (160g)",
                "1 colher de sopa de fermento em pó"
            ],
            "Recheio": [
                "1 lata de leite condensado",
                "2 caixinhas de creme de leite",
                "1 gema",
                "1/2 xícara de leite em pó",
                "150g de cream cheese"
            ],
            "Calda": [
                "200ml de água filtrada",
                "1 colher de sopa de leite em pó",
                "ou água com leite condensado"
            ],
            "Redução de Frutas Vermelhas:": [
                "450g de frutas vermelhas (amora, framboesa, mirtilo e morango)",
                "3 colheres de sopa de açúcar",
                "3 colheres de sopa de água (somente se as frutas forem frescas)"
            ]
        },
        "modo_preparo": {
            "Massa": [
                "Preaqueça o forno a 180°C.",
                "Em uma tigela, bata os ovos com o açúcar até obter um creme claro.",
                "Adicione a baunilha, o óleo e o leite, misturando bem.",
                "Incorpore a farinha de trigo aos poucos.",
                "Finalize adicionando o fermento e misture delicadamente.",
                "Despeje a Massa em forma untada e enfarinhada.",
                "Asse por 40 a 45 minutos ou até passar no teste do palito."
            ],
            "Recheio": [
                "Em uma panela, misture o leite condensado, o creme de leite, a gema e o leite em pó.",
                "Leve ao fogo médio, mexendo sempre até engrossar.",
                "Desligue o fogo e acrescente o cream cheese, misturando até ficar homogêneo.",
                "Reserve até esfriar."
            ],
            "Redução de Frutas Vermelhas": [
                "Coloque as frutas vermelhas e o açúcar em uma panela.",
                "Se as frutas forem frescas, adicione a água.",
                "Leve ao fogo médio até formar uma calda levemente espessa.",
                "Reserve para esfriar."
            ],
            "Montagem": [
                "Desenforme o bolo já frio e corte ao meio.",
                "Regue com a calda escolhida.",
                "Recheie com o creme e adicione a redução de frutas vermelhas.",
                "Cubra com a outra parte do bolo ou finalize como preferir."
            ]
        }
    },

    // === Receita 2: Sorvete de Ameixa ===
    {
        "id": 2,
        "nome": "Sorvete de Ameixa",
        "categoria": "Sobremesa",
        "tags": [
            "#doce",
            "#fruta",
            "#sorvete",
            "#gelado",
        ],
        "forma": "Pote ou forma para freezer",
        "rendimento": "aprox. 1,5 litro",
        "tempo_preparo": {
            "preparo": "20 minutos",
            "forno": "não se aplica",
            "total": "4 a 6 horas (incluindo congelamento)"
        },
        "ingredientes": {
            "Massa": [
                "1 litro de leite",
                "2 gemas",
                " 1 colher de sobremesa de manteiga",
                "2 colheres de sopa de maizena",
                "1 colher de licor de cacau",
                "Açúcar a gosto",
                "100g de ameixa",
                "1 caixinha de creme de leite",
                "1 colher de chá de essência de baunilha (opcional)"
            ],
        },
        "modo_preparo": {
            "Massa": [
                "Fazer um doce de ameixa",
            ],
            "Como Montar": [
                " Passar no liquidificador, misturar ao creme e colocar as 2 claras já em neve. Leve ao congelador ( bater  duas Vezes).",
            ],

        }
    },

    // === Receita 3: Lagarto Marinado ===
    {
        "id": 3,
        "nome": "Lagarto Marinado",
        "categoria": "Carnes",
        "tags": [
            "#salgado",
            "#lagarto",
            "#assado",
        ],
        "Panela": " sim ",
        "rendimento": "10 pessoas",
        "tempo_preparo": {
            "preparo": "15 a 20 minutos",
            "marinado": '8 a 12 horas',
            "Cocção": " 1h30 a 2h",
            "total": " 10 a 14 horas"
        },
        "ingredientes": {
            "carne": [
                "2 quilos de lagarto em bife",
                "2 cebolas",
                "2 pimentões",
                " 1 limão",
                "pimenta do reino",
                "sal",
                "alho",
                "louro",
                "salsa",
                "cebola verde",
                " orégano",
            ],
            "Molho": [
                "1 copo de vinagre",
                "1 copo de vinho branco ou madeira",
                "pimenta do reino",
                "sal",
                "orégano",
                "4 cebolas picada",
                "tomate",
                "pimentão verde",
                "salsa",
                "pimenta calbreza",
            ],
        },
        "modo_preparo": {
            "carne": [
                'Coloque a carne para cozinhar com todos os temperos, arrumando cada bife bem comprimido ao outro. Tampe e deixe cozinhar lentamente e se possível pingue água.'
            ],
            "Molho": [
                'Quando a carne estiver  cozida retire da panela e coloque neste Molho.  '
            ],
            "Montagem": [
                'Tanto se serve frio como quente.'
            ]
        }
    },

    // === Receita 4: Carne a Ingleza ===
    {
        "id": 4,
        "nome": "Carne a Ingleza",
        "categoria": "Carnes",
        "tags": [
            "#salgado",
            "#carne",
            "#almoco",
            "#jantar",
            "#lagarto",
            "#assado",
        ],
        "forma": "",
        "rendimento": "15 pessoas",
        "tempo_preparo": {
            "preparo": " 20 a 30 minutos",
            "Cocção": " 2 a 2h30",
            "Resfriamento (para fatiar)": " 2 a 4 horas",
            "total": "4h30 a 7h"
        },
        "ingredientes": {
            "carne": [
                "3 quilos de lagarto",
                " 1/2 quilo de cebolas pequenas",
                "250g de cenoura cozida",
                "1 envelope  de bacon",
                "3 ovos",
                "Farinha de mandioca",
                "Sal",
                'farofa com manteiga e duas cebolas picadas'
            ],
        },
        "modo_preparo": {
            "carne": [
                'Corte o lagarto ao meio. Tempere com sal. A parte faça uma farofa  com manteiga e duas cebolas picadas, abra a carne e espalhe o bacon, ponha a farofa e feche amarrando apertado. Coloque na panela de pressão até ficar macia.'
            ],
            "Montagem": [
                'Sirva com as cenourase cebolas cozidas.'
            ]
        }
    },

    // === Receita 5: Torta mexida ===
    {
        "id": 5,
        "nome": "Torta mexida",
        "categoria": "Salgado",
        "tags": [
            "#torta",
            "#salgado",
            "#camarão",
            "#forno",
            "#lanche"
        ],
        "forma": "Forma média retangular ou redonda untada",
        "rendimento": "8 a 12 fatias",
        "tempo_preparo": {
            "preparo": "15 a 20 minutos",
            "forno": "40 a 50 minutos",
            "total": "55 a 1h10"
        },
        "ingredientes": {
            "Massa": [
                "1 lata de ervilha",
                "1 lata de palmito",
                "1/2 quilo de camarão (pescal)",
                "1 tomate picado",
                "1 cebola batida",
                "1 punhado de cheiro verde",
                "alho a vontade",
                "2 ovos crus inteiros",
                "1 xícara  de leite",
                "1 xícara de queijo ralado",
                '1 xícara de óleo',
                "1 colher de sobremesa de fermento em pó (Pó Royal)"
            ],

        },
        "modo_preparo": {
            "Massa": [
                'Misture tudo junto, por 1 colher de sobremesa de pó royal. Untar uma forma e levar ao forno já quente'
            ],

        }
    },

    // === Receita 6: Lombo marinado ===
    {
        "id": 6,
        "nome": "Lombo marinado",
        "categoria": "Carnes",
        "tags": [
            "#salgado",
            "#carne",
            "#almoco",
            "#jantar",
            "#lombo",
            "#assado",
        ],
        "forma": "Assadeira grande",
        "rendimento": "6 a 8 porções",
        "tempo_preparo": {
            "preparo": "20 minutos",
            "coccao": "1h30",
            "total": "aprox. 14 horas (incluindo marinada)"
        },
        "ingredientes": {
            "marinada": [
                "1,5 kg de lombo",
                "3 xícaras de vinho branco seco",
                "1/2 xícara de suco de limão",
                "2 folhas de louro",
                "3 ramos de salsa",
                "3 cebolas verdes",
                "2 dentes de alho aMassados",
                "Sal a gosto",
                "Pimenta a gosto",
                "Pimenta-do-reino a gosto"
            ],
            "preparo": [
                "1 xícara de óleo",
                "1 xícara de vinho branco seco",
                "2 xícaras de caldo de carne",
                "2 cebolas em rodelas",
                "250 g de cogumelos",
                "1 colher de sopa de manteiga",
                "1 colher de sopa de farinha de trigo",
                "Pães em fatias torrados com manteiga"
            ]


        },
        "modo_preparo": {
            "Preparo Lombo": [
                "Limpe o lombo e coloque-o em uma travessa funda.",
                "Fure toda a superfície com a ponta de um garfo."
            ],
            "Marinada": [
                "Prepare a marinada.",
                "Despeje a marinada sobre o lombo, esfregando bem os temperos.",
                "Cubra com um pano ou filme plástico e deixe marinar por 12 horas."
            ],
            "Assamento": [
                "Retire o lombo da marinada.",
                "Coloque-o em forno preaquecido, regado com óleo, e asse por aproximadamente 1h30.",
                "Durante o cozimento, regue o lombo com a mistura de óleo e caldo de carne."
            ],
            "Molho": [
                "Asse ou salteie os cogumelos na manteiga, temperando com sal.",
                "Leve a marinada ao fogo e deixe ferver por alguns minutos para reduzir levemente. Reserve.",
                "Quando o lombo estiver cozido, retire parte da gordura da assadeira.",
                "Na gordura restante, dissolva a farinha de trigo.",
                "Acrescente os cogumelos (e a cenoura, se utilizada) e leve ao fogo por cerca de 2 minutos."
            ],
            "Finalização": [
                "Coloque o lombo de volta na assadeira.",
                "Cubra com o Molho.",
                "Sirva quente, decorado com as torradas."
            ]
        }

    },

    // === Receita 7: Purê de Castanhas Portuguesas ===
    {
        "id": 7,
        "nome": "Purê de Castanhas Portuguesas",
        "categoria": "Acompanhamento",
        "tags": [
            "#salgado",
            "#acompanhamento",
            "#castanhas",
            "#purê",
            "#inverno"
        ],
        "forma": "",
        "rendimento": "8 pessoas",
        "tempo_preparo": {
            "preparo": "20 minutos",
            "cocção": "15 minutos",
            "total": "35 minutos"
        },
        "ingredientes": {
            "Massa": [
                "1 kg de castanhas portuguesas cozidas e descascadas",
                "3 colheres de sopa de manteiga",
                "1 xícara de leite",
                "1/2 lata de creme de leite",
                "Sal a gosto",
                "Pimenta-do-reino a gosto"
            ],

        },
        "modo_preparo": {
            "Massa": [
                "Passe as castanhas ainda quentes pelo espremedor ou amasse bem até obter um purê.",
                "Em uma panela, misture o purê de castanhas com a manteiga e o leite.",
                "Leve ao fogo médio, mexendo sempre, até obter um purê liso, homogêneo e consistente.",
                "Adicione o creme de leite, tempere com sal e pimenta-do-reino a gosto.",
                "Continue mexendo por alguns minutos, apenas até aquecer bem."
            ],
            "Montagem": [
                "Sirva quente como acompanhamento."
            ]
        }
    },

    // === Receita 8: Mil-folhas Salgado ===
    {
        "id": 8,
        "nome": "Mil-folhas Salgado",
        "categoria": "Prato Principal",
        "tags": [
            "#salgado",
            "#Massa",
            "#forno",
            "#presunto",
            "#mussarela"
        ],
        "forma": "Assadeira retangular",
        "rendimento": "6 a 8 porções",
        "tempo_preparo": {
            "preparo": "30 minutos",
            "cocção": "30 minutos",
            "total": "60 minutos"
        },
        "ingredientes": {
            "Massa": [
                "1/2 receita de Massa folhada congelada"
            ],
            "Recheio": [
                "200 g de presunto em fatias",
                "200 g de mussarela ralada"
            ],
            "Molho": [
                "1 colher de sopa de manteiga",
                "5 cebolinhas pequenas picadas",
                "1 xícara de chá de cogumelos picados",
                "1 colher de sopa de farinha de trigo",
                "1 1/2 xícara de leite",
                "1/2 xícara de creme de leite",
                "Sal a gosto",
                "Pimenta-do-reino a gosto",
                "Cebolinha verde picada a gosto"
            ]
        },
        "modo_preparo": {
            "Massa": [
                "Abra a Massa folhada com espessura aproximada de 3 cm.",
                "Corte em três pedaços retangulares iguais.",
                "Coloque-os em uma assadeira levemente molhada, fure com um garfo e leve ao forno preaquecido a 180 °C por 15 minutos."
            ],
            "Molho": [
                "Aqueça a manteiga em uma panela e refogue as cebolinhas até dourarem levemente.",
                "Acrescente os cogumelos e refogue por alguns minutos.",
                "Polvilhe a farinha de trigo e mexa bem.",
                "Despeje o leite aos poucos, mexendo sempre, até obter um Molho liso.",
                "Tempere com sal e pimenta.",
                "Junte o creme de leite e a cebolinha verde.",
                "Misture bem e deixe esfriar."
            ],
            "Montagem": [
                "Coloque um retângulo de Massa em um refratário.",
                "Cubra com presunto, um pouco de mussarela e parte do Molho.",
                "Coloque o segundo retângulo de Massa e repita as camadas.",
                "Finalize com o terceiro retângulo de Massa, comprimindo levemente.",
                "Espalhe o restante do Molho e cubra com a mussarela."
            ],

        },
        " Finalização": [
            "Leve ao forno novamente por 15 minutos, até aquecer bem e gratinar.",
            "Sirva quente."
        ]
    },

    // === Receita 9: Empadão de Frango ===
    {
        "id": 9,
        "nome": "Empadão de Frango",
        "categoria": "Salgado",
        "tags": [
            "#salgado",
            "#forno",
            "#frango",
            "#empadão",
            "#Massa"
        ],
        "forma": "Forma de aro removível",
        "rendimento": "6 a 8 porções",
        "tempo_preparo": {
            "preparo": "40 minutos",
            "cocção": "45 minutos",
            "total": "1 hora e 25 minutos"
        },
        "ingredientes": {
            "Massa": [
                "250 g de farinha de trigo",
                "1 ovo",
                "1 colher de sopa de manteiga",
                "1 colher de sopa de banha",
                "Sal a gosto"
            ],
            "Recheio": [
                "1 frango pequeno cozido e desfiado",
                "Caldo do cozimento do frango",
                "1 colher de sopa de farinha de trigo",
                "1 gema",
                "Ovos cozidos picados a gosto",
                "Ervilhas a gosto"
            ]
        },
        "modo_preparo": {
            "Massa": [
                "Faça um anel com farinha de trigo sobre uma superfície.",
                "Coloque no centro o ovo, o sal e a banha.",
                "Misture bem e acrescente a manteiga.",
                "Trabalhe a Massa até ficar homogênea e no ponto de abrir.",
                "Enrole a Massa em um pano úmido e deixe descansar por 30 minutos."
            ],
            "Recheio": [
                "Cozinhe o frango e desfie.",
                "Engrosse o caldo do cozimento com farinha de trigo.",
                "Acrescente a gema, mexendo bem.",
                "Junte o frango desfiado, os ovos cozidos e as ervilhas.",
                "Deixe o Recheio esfriar completamente antes de usar."
            ],
            "Montagem": [
                "Forre uma forma de aro removível com parte da Massa.",
                "Coloque o Recheio frio.",
                "Cubra com outra camada de Massa, não muito grossa.",
                "Decore com pedaços da Massa.",
                "Pincele com gema ou leite."
            ]
        },
        " Finalização": [
            "Leve ao forno preaquecido a 180 °C até dourar.",
            "Sirva quente ou morno."
        ]
    },

    // === Receita 10: Frango ao Ketchup ===
    {
        "id": 10,
        "nome": "Frango ao Ketchup",
        "categoria": "Prato Principal",
        "tags": [
            "#salgado",
            "#frango",
            "#forno",
            "#assado",
            "#Molho"
        ],
        "forma": "Refratário",
        "rendimento": "6 porções",
        "tempo_preparo": {
            "preparo": "15 minutos",
            "cocção": "45 a 60 minutos",
            "total": "1 hora a 1 hora e 15 minutos"
        },
        "ingredientes": {
            "Principal": [
                "1,5 kg de frango (peito ou coxa)",
                "Sal a gosto"
            ],
            "Molho": [
                "1 xícara de chá de ketchup picante",
                "1 colher de sopa de mostarda"
            ]
        },
        "modo_preparo": {
            "Principal": [
                "Lave bem o frango e tempere com sal.",
                "Unte um refratário com manteiga.",
                "Disponha os pedaços de frango no refratário."
            ],
            "Molho": [
                "Misture o ketchup com a mostarda até ficar homogêneo.",
                "Espalhe o Molho sobre o frango."
            ],
            " Finalização": [
                "Leve ao forno preaquecido a 180 °C por 45 minutos a 1 hora, até dourar e ficar bem assado.",
                "Sirva quente."
            ]
        }
    },

    // === Receita 11: Ovos Assados ===
    {
        "id": 11,
        "nome": "Ovos Assados",
        "categoria": "Entrada",
        "tags": [
            "#entrada",
            "#ovos",
            "#forno",
            "#individual",
            "#queijos"
        ],
        "forma": "Refratários individuais",
        "rendimento": "1 porção por unidade",
        "tempo_preparo": {
            "preparo": "10 minutos",
            "cocção": "30 minutos",
            "total": "40 minutos"
        },
        "ingredientes": {
            "Principal": [
                "Ovos (1 por porção)",
                "Manteiga para untar",
                "Mussarela fatiada ou ralada suficiente para forrar o refratário",
                "2 colheres de sopa de leite por ovo",
                "Sal a gosto",
                "Pimenta-do-reino a gosto",
                "Queijo parmesão ralado a gosto"
            ]
        },
        "modo_preparo": {
            "Principal": [
                "Unte refratários individuais com manteiga.",
                "Forre o fundo e as laterais com mussarela.",
                "Quebre um ovo em cada refratário.",
                "Misture o leite com sal e pimenta-do-reino e despeje sobre o ovo.",
                "Polvilhe queijo parmesão ralado por cima.",
                "Leve ao forno preaquecido a 180 °C por cerca de 30 minutos, até assar."
            ]
        },
        " Finalização": [
            "Sirva quente como entrada."
        ]
    },

    // === Receita 12 (Emapdinha de queijo) ===
    {
        "id": 12,
        "nome": "Empadinha de Queijo",
        "categoria": "Salgado",
        "tags": [
            "#salgado",
            "#empadinha",
            "#queijo",
            "#forno",
            "#lanche"
        ],
        "forma": "Forminhas de empada",
        "rendimento": "20 a 24 empadinhas",
        "tempo_preparo": {
            "preparo": "25 minutos",
            "cocção": "20 minutos",
            "total": "45 minutos"
        },
        "ingredientes": {
            "Massa": [
            "7 colheres de sopa de farinha de trigo",
            "1 colher de sopa bem cheia de banha",
            "1 colher de sopa bem cheia de margarina",
            "2 gemas",
            "1 colher de sopa de leite",
            "1 colher de café de sal"
            ],
            "Recheio": [
            "2 xícaras de chá de queijo mineiro ralado",
            "1 xícara de chá de queijo parmesão ralado",
            "1/2 a 1 copo de leite",
            "1 colher de sopa de margarina",
            "2 ovos",
            "Sal a gosto"
            ]
        },
        "modo_preparo": {
            "Massa": [
            "Em uma tigela, misture a farinha de trigo e o sal.",
            "Acrescente a banha e a margarina e misture com a ponta dos dedos até formar uma farofa.",
            "Junte as gemas e o leite aos poucos, misturando até obter uma Massa lisa e macia.",
            "Cubra a Massa e deixe descansar por 15 minutos."
            ],
            "Recheio": [
            "Em uma tigela, misture os queijos ralados.",
            "Acrescente os ovos, a margarina e misture bem.",
            "Adicione o leite aos poucos até obter um Recheio cremoso.",
            "Ajuste o sal, se necessário."
            ],
            "Montagem": [
            "Unte levemente as forminhas de empada.",
            "Forre o fundo e as laterais com a Massa.",
            "Distribua o Recheio, sem cobrir com Massa."
            ]
        },
        " Finalização": [
            "Leve ao forno bem quente (200–220 °C) até dourar.",
            "Retire do forno e sirva quente ou morna."
        ]
}
,

    // === Receita 13 (Bobó de camarao) ===
        {
    "id": 13,
    "nome": "Bobó de Camarão",
    "categoria": "Prato Principal",
    "tags": [
        "#frutosdomar",
        "#camarao",
        "#mandioca",
        "#regional",
        "#salgado"
    ],
    "forma": "",
    "rendimento": "8 a 10 porções",
    "tempo_preparo": {
        "preparo": "40 minutos",
        "cocção": "40 minutos",
        "total": "1 hora e 20 minutos"
    },
    "ingredientes": {
        "Principal": [
        "2 kg de camarão limpo",
        "1 kg de mandioca",
        "1 litro de creme de leite",
        "1 vidro de leite de coco",
        "Ketchup a gosto",
        "Azeite de dendê a gosto"
        ]
    },
    "modo_preparo": {
        "Principal": [
        "Cozinhe a mandioca em panela separada com temperos a gosto até ficar bem macia e começar a desmanchar.",
        "Em outra panela, refogue o camarão com temperos a gosto até ficar bem cozido.",
        "Misture o camarão refogado à mandioca cozida, formando uma mistura homogênea.",
        "Acrescente o leite de coco e o ketchup, misturando bem.",
        "Ajuste os temperos, se necessário, e deixe aquecer bem."
        ]
    },
    " Finalização": [
        "No momento de servir, acrescente o azeite de dendê e o creme de leite.",
        "Misture delicadamente e sirva quente."
    ]
    },
// === Receita 14 (Torta para lanche) ===
    {
    "id": 14,
    "nome": "Torta para Lanche (sem Recheio)",
    "categoria": "Lanche / Salgado",
    "tags": [
        "#lanche",
        "#torta",
        "#liquidificador",
        "#forno",
        "#salgado"
    ],
    "forma": "Forma média",
    "rendimento": "8 a 10 porções",
    "tempo_preparo": {
        "preparo": "15 minutos",
        "cocção": "35 a 40 minutos",
        "total": "50 a 55 minutos"
    },
    "ingredientes": {
        "Massa": [
        "5 ovos",
        "1 xícara de chá de leite",
        "1 xícara de chá de óleo",
        "2 xícaras de chá de farinha de trigo",
        "1 colher de sopa de fermento em pó (Royal)",
        "5 colheres de sopa de queijo ralado",
        "Sal a gosto"
        ],
        "Recheio": [
        "Recheio de sua preferência (presunto e queijo, frango, legumes, atum, etc.)",
        "Queijo ralado a gosto para finalizar"
        ]
    },
    "modo_preparo": {
        "Massa": [
        "No liquidificador, bata os ovos, o leite, o óleo, o sal e o queijo ralado.",
        "Acrescente a farinha de trigo aos poucos e bata até obter uma Massa homogênea.",
        "Por último, adicione o fermento em pó e misture delicadamente."
        ],
        "Montagem": [
        "Unte a forma com óleo e polvilhe farinha de trigo.",
        "Despeje metade da Massa na forma.",
        "Distribua o Recheio escolhido.",
        "Cubra com o restante da Massa.",
        "Polvilhe queijo ralado por cima."
        ]
    },
    " Finalização": [
        "Leve ao forno morno (180 °C) até dourar e assar por completo.",
        "Retire do forno, deixe amornar e sirva."
    ]
    },
    // === Receita 15: Pizza Caseira ===
// === Receita 15: Pizza Caseira ===
{
    id: 15,
    nome: "Pizza Caseira",
    categoria: "Salgado",
    tags: [
        "#salgado",
        "#massa",
        "#forno",
        "#lanche",
        "#caseira"
    ],

    forma: "Forma de pizza",
    "rendimento": "4 a 6 porções",
    // 🔹 Quantidade estimada (não altera a receita)
    porcoes: {
        pessoas: "4 a 6",
        unidades: "1 pizza grande"
    },

    // 🔹 Tempo organizado (mantendo o que foi descrito)
    tempo_preparo: {
        preparo: "20 minutos",
        forno: "20 a 30 minutos",
        cocção: "",
        marinada: "",
        Descanso: "até a massa crescer",
        total: "aprox. 1 hora"
    },

    // 
    ingredientes: {
        massa: [
        "2 tabletes de fermento",
        "1 colher de sopa de açúcar",
        "1 ovo",
        "1 xícara de café de óleo",
        "1 copo de leite",
        "Sal a gosto",
        "Farinha de trigo o suficiente para dar ponto"
        ],
        recheio: [
        "Queijo",
        "Tomate",
        "Cebola",
        "Azeite",
        "Sal",
        "Pimenta-do-reino",
        "Salsa",
        "Orégano",
        "Molho de tomate"
        ]
    },

    // 🔹 MODO DE PREPARO com texto corrigido e organizado
    modo_preparo: {
        Massa: [
        "Coloque o ovo, o fermento, o açúcar e o sal em uma tigela.",
        "Aqueça levemente o leite e misture aos ingredientes.",
        "Acrescente a farinha de trigo aos poucos, mexendo até a massa desgrudar da mão.",
        "Unte a forma com óleo e abra a massa."
        ],
        Descanso: [
        "Deixe a massa descansar até crescer."
        ],
        'Pré Assamento': [
        "Leve a massa ao forno brando para pré-assar, furando-a com um garfo."
        ],
        Montagem: [
        "Retire do forno, espalhe o molho de tomate e distribua o recheio.",
        "Leve novamente ao forno até assar completamente."
        ]
    }
}
,

// === Receita 16: Canapés de Pepino ===
{
    id: 16,
    nome: "Canapés de Pepino",
    categoria: "Entrada",
    tags: [
        "#salgado",
        "#entrada",
        "#pepino",
        "#frio",
        "#aperitivo"
    ],

    // Forma não se aplica, mas mantida para padronização
    forma: "Não se aplica",
    "rendimento": "6 a 8 porções",
    // 🔹 Quantidade estimada com base no tipo de preparo
    porcoes: {
        pessoas: "6 a 8",
        unidades: "aprox. 20 a 25 canapés"
    },

    // 🔹 Tempo não informado na receita original
    tempo_preparo: {
        preparo: "15 minutos",
        forno: "",
        cocção: "",
        marinada: "",
        Descanso: "",
        total: "15 minutos"
    },

    // 🔹 INGREDIENTES (texto corrigido, conteúdo mantido)
    ingredientes: {
        base: [
        "Pão de forma cortado em rodelas (não utilizar pão tipo Pullman)"
        ],
        creme: [
        "Maionese",
        "Mostarda"
        ],
        cobertura: [
        "Pepino sem casca fatiado",
        "Alcachofras enroladas"
        ]
    },

    // 🔹 MODO DE PREPARO com ortografia e concordância corrigidas
    modo_preparo: {
        preparo: [
        "Corte o pão de forma em rodelas pequenas e redondas.",
        "Misture partes iguais de maionese e mostarda até obter um creme homogêneo.",
        "Passe o creme sobre as fatias de pão.",
        "Cubra cada fatia com uma rodela de pepino sem casca.",
        "Finalize colocando as alcachofras enroladas por cima."
        ]
    }
},
// === Receita 17: Torta de Batata ===
{
    id: 17,
    nome: "Torta de Batata",
    categoria: "Salgado",
    tags: [
        "#salgado", // 🔹 TAG OBRIGATÓRIA
        "#torta",
        "#batata",
        "#forno",
        "#lanche"
    ],

    forma: "Forma média untada",
    "rendimento": "6 a 8 porções",
    porcoes: {
        pessoas: "6 a 8",
        unidades: ""
    },

    tempo_preparo: {
        preparo: "25 minutos",
        forno: "35 a 40 minutos",
        cocção: "",
        marinada: "",
        Descanso: "",
        total: "1 hora"
    },

    // 🔹 INGREDIENTES com escrita corrigida
    ingredientes: {
        massa: [
        "8 batatas grandes já cozidas com sal",
        "2 colheres de sopa de farinha de trigo bem cheias",
        "2 colheres de sopa de queijo ralado",
        "3 gemas",
        "1 colher de chá de fermento em pó (Pó Royal)",
        "1 colher de chá de açúcar",
        "Pimenta-do-reino a gosto"
        ],
        finalização: [
        "Claras em neve",
        "Queijo ralado para finalizar",
        "1 gema batida para pincelar"
        ]
    },

    // 🔹 MODO DE PREPARO com concordância corrigida
    modo_preparo: {
        massa: [
        "Passe as batatas ainda mornas pelo espremedor e deixe esfriar, mexendo sempre.",
        "Após esfriar, acrescente a farinha de trigo, o queijo ralado, as gemas, o fermento, o açúcar e a pimenta-do-reino.",
        "Misture bem até obter uma massa homogênea.",
        "Por último, incorpore delicadamente as claras em neve."
        ],
        Montagem: [
        "Unte bem a forma.",
        "Espalhe a massa na forma com o auxílio de uma colher.",
        "Coloque o recheio de sua preferência.",
        "Cubra com queijo ralado.",
        "Pincele com a gema batida."
        ]
    },

    finalização: [
        "Leve ao forno preaquecido a 180 °C até dourar.",
        "Retire do forno e sirva quente ou morna."
    ]
    },
    // === Receita 18: Pão de Queijo ===
{
    id: 18,
    nome: "Pão de Queijo",
    categoria: "Salgado",
    tags: [
        "#salgado", // 🔹 TAG OBRIGATÓRIA
        "#paoDeQueijo",
        "#forno",
        "#lanche",
        "#mineiro"
    ],

    forma: "Assadeira untada",
    "rendimento": "8 a 10 porções",
    porcoes: {
        pessoas: "8 a 10",
        unidades: "aprox. 25 a 30 unidades"
    },

    tempo_preparo: {
        preparo: "30 minutos",
        forno: "25 a 30 minutos",
        cocção: "",
        marinada: "",
        descanso: "até a massa esfriar",
        total: "aprox. 1 hora"
    },

    // 🔹 INGREDIENTES com ortografia corrigida
    ingredientes: {
        massa: [
        "1 prato fundo de polvilho doce bem cheio",
        "1 copo de leite",
        "1 prato fundo de água",
        "1 prato fundo de óleo",
        "Erva-doce a gosto",
        "1 prato fundo de queijo ralado",
        "Sal a gosto",
        "7 ovos"
        ]
    },

    // 🔹 MODO DE PREPARO com escrita corrigida
    modo_preparo: {
        preparo: [
        "Leve ao fogo o leite, o óleo, a água, a erva-doce e o sal até ferver.",
        "Escalde o polvilho doce com essa mistura e deixe esfriar.",
        "Após esfriar, amasse a massa acrescentando 5 ovos.",
        "Junte o queijo ralado e misture bem.",
        "Adicione os 2 ovos restantes, um a um, até dar o ponto."
        ]
    },

    finalizacao: [
        "Modele os pães de queijo.",
        "Disponha em assadeira untada.",
        "Leve ao forno preaquecido a 180 °C até dourar."
    ]
},
// === Receita 19: Macarrão Assado ===
{
  id: 19,
  nome: "Macarrão Assado",
  categoria: "Salgado",
  tags: [
    "#salgado", // 🔹 TAG OBRIGATÓRIA
    "#macarrao",
    "#forno",
    "#massa",
    "#almoco"
  ],

  forma: "Refratário médio untado",
  "rendimento": "6 a 8 porções",
  porcoes: {
    pessoas: "6 a 8",
    unidades: ""
  },

  tempo_preparo: {
    preparo: "25 minutos",
    forno: "30 a 40 minutos",
    cocção: "",
    marinada: "",
    descanso: "",
    total: "1 hora"
  },

  // 🔹 INGREDIENTES com escrita corrigida
  ingredientes: {
    principal: [
      "1 pacote de macarrão",
      "1 lata de creme de leite",
      "2 ovos crus",
      "Maionese a gosto",
      "Molho de tomate a gosto",
      "Presunto picado",
      "Queijo ralado ou fatiado"
    ]
  },

  // 🔹 MODO DE PREPARO criado a partir dos ingredientes informados
  modo_preparo: {
    preparo: [
      "Cozinhe o macarrão em água e sal até ficar al dente.",
      "Escorra bem o macarrão e reserve.",
      "Em uma tigela, misture o creme de leite, os ovos crus, a maionese e parte do molho de tomate.",
      "Acrescente o presunto e misture bem."
    ],
    montagem: [
      "Unte um refratário com um pouco de óleo ou margarina.",
      "Misture o macarrão ao creme preparado.",
      "Coloque a mistura no refratário.",
      "Cubra com o restante do molho de tomate e espalhe o queijo por cima."
    ]
  },

  finalizacao: [
    "Leve ao forno preaquecido a 180 °C por 30 a 40 minutos, até dourar.",
    "Retire do forno e sirva quente."
  ]
},
// === Receita 20: Vatapá ===
{
  id: 20,
  nome: "Vatapá",
  categoria: "Salgado",
  tags: [
    "#salgado", // 🔹 TAG OBRIGATÓRIA
    "#vatapa",
    "#cozinhaBrasileira",
    "#baiana",
    "#pratoPrincipal"
  ],
  "rendimento": "10 a 12 porções",
  'Porções': {
    pessoas: "10 a 12",
    unidades: ""
  },

  tempo_preparo: {
    preparo: "1 hora",
    cocção: "1 hora",
    descanso: "",
    total: "2 horas"
  },

  // 🔹 INGREDIENTES (texto corrigido, sem alterar a receita)
  ingredientes: {
    Base: [
      "1 galinha inteira",
      "Água o quanto baste",
      "1 pacote de camarão congelado",
      "Sal a gosto",
      "3 cebolas picadas",
      "1 concha de azeite",
      "Pimenta-do-reino a gosto",
      "Pimenta vermelha a gosto",
      "1 kg de tomates batidos no liquidificador",
      "1 vidro de palmito picado"
    ],
    Engrossar: [
      "2 pães embebidos em 1 litro de leite",
      "1 vidro de leite de coco",
      "1 lata de castanhas",
      "200 g de caju batido no liquidificador com leite aos poucos",
      "1/2 vidro de azeite de dendê"
    ],
    'Creme arroz': [
      "1 caixa de creme de arroz (200 g)",
      "1 litro de leite",
      "1 litro de leite de coco",
      "Sal a gosto"
    ]
  },

  // 🔹 MODO DE PREPARO com correções
  modo_preparo: {
    'Preparo base': [
      "Cozinhe a galinha em água e sal. Reserve o caldo.",
      "Depois de fria, retire a carne dos ossos e reserve.",
      "Leve os ossos e a pele para ferver com um pouco de água.",
      "Coe esse caldo e reserve junto com o caldo da galinha."
    ],
    'Preparo camarao': [
      "Limpe e afervente o camarão.",
      "Refogue o camarão com sal, as cebolas picadas, o azeite, a pimenta-do-reino e a pimenta vermelha.",
      "Acrescente os tomates batidos no liquidificador.",
      "Junte o camarão refogado, o caldo da galinha e o caldo dos ossos coado.",
      "Adicione o palmito picado e a água, se necessário."
    ],
    Engrossar: [
      "Bata no liquidificador os pães embebidos no leite.",
      "Misture esse creme aos outros ingredientes para engrossar.",
      "Deixe cozinhar por cerca de 20 minutos, mexendo sempre.",
      "Retifique o sal e os temperos.",
      "Acrescente o leite de coco e deixe ferver mais um pouco.",
      "Adicione as castanhas e o caju batido com leite.",
      "Misture bem, retire do fogo e acrescente o azeite de dendê."
    ],
    'Creme de arroz': [
      "Dissolva o creme de arroz no leite.",
      "Leve ao fogo junto com o leite de coco e o sal.",
      "Cozinhe mexendo até obter um creme homogêneo."
    ]
  },

  'Finalização': [
    "Misture o creme de arroz ao vatapá ainda quente.",
    "Sirva acompanhado de arroz branco."
  ]
},
// === Receita 21: Mousse de Atum ===
{
  id: 21,
  nome: "Mousse de Atum",
  categoria: "Salgado",
  tags: [
    "#salgado", // 🔹 TAG OBRIGATÓRIA
    "#atum",
    "#mousseSalgado",
    "#entrada",
    "#geladeira"
  ],

  forma: "Forma untada",
  "rendimento": "8 a 10 porções",
  'porções': {
    pessoas: "8 a 10",
    unidades: ""
  },

  tempo_preparo: {
    preparo: "20 minutos",
    geladeira: "12 horas",
    descanso: "",
    total: "aprox. 12 horas e 20 minutos"
  },

  // 🔹 INGREDIENTES com escrita corrigida
  ingredientes: {
    base: [
      "2 latas de atum (sem o caldo)",
      "1 lata de creme de leite (sem o soro)",
      "1 xícara de chá de ketchup",
      "1 xícara de chá de picles picados finos",
      "1 vidro pequeno de maionese",
      "6 folhas de gelatina branca dissolvidas em água quente",
      "Mostarda a gosto",
      "Molho inglês a gosto",
      "Sal a gosto"
    ]
  },

  // 🔹 MODO DE PREPARO com correções
  Modo_preparo: {
    Preparo: [
      "Em uma tigela, misture o atum, o creme de leite, o ketchup, os picles e a maionese.",
      "Acrescente a mostarda, o molho inglês e o sal.",
      "Misture bem até obter um creme homogêneo.",
      "Adicione a gelatina já dissolvida e mexa novamente."
    ],
    Montagem: [
      "Unte uma forma com óleo.",
      "Despeje a mistura na forma.",
      "Leve à geladeira até ficar bem firme."
    ]
  },

  Finalizacao: [
    "Sirva no dia seguinte.",
    "Acompanhe com torradas."
  ]
},
// === Receita 22: Panquecas ===
{
  id: 22,
  nome: "Panquecas",
  categoria: "Salgado",
  tags: [
    "#salgado", // 🔹 TAG OBRIGATÓRIA
    "#panqueca",
    "#massa",
    "#lanche"
  ],

  forma: "Frigideira untada",
  "rendimento": "4 porções",
  porções: {
    pessoas: "4",
    unidades: "8 a 10 panquecas"
  },

  tempo_preparo: {
    preparo: "10 minutos",
    cocção: "20 minutos",
    descanso: "",
    total: "30 minutos"
  },

  ingredientes: {
    massa: [
      "1 copo de leite",
      "1 copo de farinha de trigo",
      "1 ovo",
      "1 colher de sopa de óleo",
      "1/2 colher de café bem rasa de pó Royal",
      "Sal a gosto"
    ]
  },

  modo_preparo: {
    preparo: [
      "Bata no liquidificador o leite, a farinha de trigo, o ovo, o óleo, o pó Royal e o sal.",
      "Misture até obter uma massa homogênea."
    ],
    cocção: [
      "Unte levemente uma frigideira.",
      "Leve ao fogo médio.",
      "Coloque pequenas porções da massa.",
      "Doure dos dois lados."
    ]
  },

  finalização: [
    "Utilize o recheio de sua preferência.",
    "Sirva quente."
  ]
},
// === Receita 23: Kibe ===
{
  id: 23,
  nome: "Kibe",
  categoria: "Salgado",
  tags: [
    "#salgado", // 🔹 TAG OBRIGATÓRIA
    "#kibe",
    "#carne",
    "#forno",
    "#arabe"
  ],

  forma: "Assadeira untada",
  "rendimento": "8 a 10 porções",
  porções: {
    pessoas: "8 a 10",
    unidades: ""
  },

  tempo_preparo: {
    preparo: "30 minutos",
    descanso: "10 minutos",
    forno: "40 minutos",
    total: "1 hora e 20 minutos"
  },

  ingredientes: {
    massa: [
      "1 kg de carne moída",
      "1/2 kg de trigo para kibe (lavar de 8 a 10 vezes)",
      "Sal a gosto",
      "Pimenta-do-reino a gosto",
      "Cebola picada a gosto",
      "Hortelã a gosto",
      "1 copo de água gelada",
      "1/2 copo de óleo"
    ],
    recheio: [
      "4 cebolas grandes picadas",
      "1/2 tablete de manteiga",
      "Sal a gosto",
      "Pimenta-do-reino a gosto"
    ]
  },

  modo_preparo: {
    'preparo massa': [
      "Lave bem o trigo para kibe e escorra.",
      "Em uma bacia, misture a carne moída, o trigo, o sal, a pimenta-do-reino, a cebola e a hortelã.",
      "Acrescente a água gelada e o óleo.",
      "Misture bem até obter uma massa homogênea."
    ],
    'preparo recheio': [
      "Refogue as cebolas na manteiga.",
      "Tempere com sal e pimenta-do-reino.",
      "Reserve."
    ],
    montagem: [
      "Unte uma assadeira com óleo.",
      "Divida a massa em duas partes.",
      "Coloque metade da massa na assadeira.",
      "Espalhe o recheio por cima.",
      "Cubra com a outra metade da massa.",
      "Molhe as mãos para espalhar melhor a massa."
    ]
  },

  finalização: [
    "Leve ao forno preaquecido a 180 °C até assar e dourar.",
    "Sirva quente."
  ]
},

// === Receita 24: Esfirra ===
{
  id: 24,
  nome: "Esfirra",
  categoria: "Salgado",
  tags: [
    "#salgado",
    "#esfirra",
    "#massa",
    "#forno",
    "#arabe"
  ],

  forma: "Assadeira untada",
  "rendimento": "10 porções",
  porções: {
    pessoas: "10",
    unidades: "20"
  },

  tempo_preparo: {
    preparo: "40 minutos",
    descanso: "1 hora",
    forno: "30 a 40 minutos",
    total: "2 horas e 20 minutos"
  },

  ingredientes: {
    massa: [
      "2 kg de farinha de trigo",
      "5 ovos",
      "2 colheres de fermento",
      "1 litro de leite",
      "4 colheres de manteiga",
      "5 colheres de sopa de açúcar",
      "1 colher de sopa de sal",
      "2 copos de óleo"
    ],
    recheio: [
      "Recheio a gosto"
    ]
  },

  modo_preparo: {
    'preparo massa': [
      "Derreta a manteiga.",
      "Acrescente o leite e as gemas.",
      "Mexa bem.",
      "Dissolva o fermento em um pouco de água.",
      "Junte o fermento à mistura.",
      "Adicione os demais ingredientes da massa.",
      "Misture bem até formar uma massa homogênea."
    ],
    descanso: [
      "Coloque a massa em um recipiente.",
      "Deixe crescer por aproximadamente 1 hora."
    ],
    montagem: [
      "Divida a massa em porções.",
      "Deixe as porções reservadas.",
      "Recheie a gosto.",
      "Disponha as esfirras em assadeira untada."
    ]
  },

  finalização: [
    "Leve ao forno preaquecido a 180 °C até dourar.",
    "Sirva quente."
  ]
}
,

// === Receita 25: Molho Branco ===
{
  id: 25,
  nome: "Molho Branco",
  categoria: "Salgado",
  tags: [
    "#salgado", // 🔹 TAG OBRIGATÓRIA
    "#molho_branco",
    "#molho",
    "#acompanhamento"
  ],

  rendimento: "Aproximadamente 1,2 litros",

  porções: {
    pessoas: "6 a 8",
    unidades: "Serve massas, legumes ou carnes"
  },

  tempo_preparo: {
    preparo: "10 minutos",
    cocção: "15 minutos",
    total: "25 minutos"
  },

  ingredientes: {
    base: [
      "1/2 tablete de margarina",
      "1 cebola ralada",
      "1 litro de leite",
      "1 colher de sopa de farinha de trigo",
      "2 cubos de caldo de carne ou de galinha",
      "1 lata de creme de leite"
    ]
  },

  modo_preparo: {
    preparo: [
      "Derreta a margarina em uma panela.",
      "Acrescente a cebola ralada e refogue.",
      "Adicione quase todo o leite.",
      "À parte, dissolva a farinha de trigo em um pouco do leite reservado.",
      "Junte essa mistura ao molho, mexendo sempre.",
      "Deixe ferver até engrossar.",
      "Acrescente os cubos de caldo de carne ou de galinha."
    ],
    finalizacao: [
      "Desligue o fogo.",
      "Acrescente o creme de leite e misture bem."
    ]
  }
}
,

// === Receita 26: Berinjela Temperada ===
{
  id: 26,
  nome: "Berinjela Temperada",
  categoria: "Salgado",
  tags: [
    "#salgado", // 🔹 TAG OBRIGATÓRIA
    "#berinjela",
    "#acompanhamento",
    "#frito"
  ],

  rendimento: "1 travessa média",

  porções: {
    pessoas: "4 a 6",
    unidades: ""
  },

  tempo_preparo: {
    preparo: "15 minutos",
    fritura: "20 minutos",
    total: "35 minutos"
  },

  ingredientes: {
    principal: [
      "3 berinjelas",
      "3 colheres de sopa de terrine",
      "3 limões médios",
      "Óleo para fritar",
      "Água"
    ],
    tempero: [
      "Alho a gosto",
      "Cebola a gosto",
      "Pimenta-do-reino a gosto"
    ]
  },

  modo_preparo: {
    preparo_berinjela: [
      "Lave bem as berinjelas.",
      "Frite as berinjelas inteiras, com casca, utilizando óleo e água.",
      "Retire e deixe escorrer."
    ],
    preparo_tempero: [
      "Misture o suco dos limões, o alho, a cebola e o terrine.",
      "Tempere com pimenta-do-reino."
    ],
    montagem: [
      "Coloque as berinjelas em um recipiente.",
      "Cubra com o tempero preparado."
    ]
  }
}
,
// === Receita 27: Bombom de Uva ===
{
  id: 27,
  nome: "Bombom de Uva",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#bombom",
    "#uva",
    "#sobremesa"
  ],

  porções: {
    pessoas: "",
    unidades: "aprox. 20 unidades"
  },

  tempo_preparo: {
    preparo: "10 minutos",
    cocção: "15 minutos",
    descanso: "20 minutos",
    total: "45 minutos"
  },

  ingredientes: {
    principal: [
      "1 lata de leite condensado",
      "1 coco ralado",
      "1 colher de manteiga",
      "2 gemas",
      "Uvas inteiras"
    ]
  },

  modo_preparo: {
    preparo: [
      "Misture o leite condensado, o coco ralado, a manteiga e as gemas.",
      "Leve ao fogo médio, mexendo sempre, até dar ponto e dourar levemente.",
      "Retire do fogo e deixe esfriar."
    ],
    montagem: [
      "Envolva as uvas com a massa já fria.",
      "Modele os bombons."
    ]
  }
},
// === Receita 28: Brigadeiro ===
{
  id: 28,
  nome: "Brigadeiro",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#brigadeiro",
    "#chocolate",
    "#sobremesa"
  ],

  porções: {
    pessoas: "",
    unidades: "aprox. 20 unidades"
  },

  tempo_preparo: {
    preparo: "5 minutos",
    cocção: "10 minutos",
    total: "15 minutos"
  },

  ingredientes: {
    principal: [
      "1 lata de leite condensado",
      "2 colheres de Nescau",
      "1 colher de manteiga"
    ]
  },

  modo_preparo: {
    preparo: [
      "Em uma panela, coloque o leite condensado, o Nescau e a manteiga.",
      "Leve ao fogo baixo, mexendo sempre.",
      "Cozinhe até a massa desgrudar do fundo da panela."
    ],
    finalização: [
      "Retire do fogo.",
      "Deixe esfriar antes de enrolar ou utilizar como recheio."
    ]
  }
},
// === Receita 29: Bom-bocado ===
{
  id: 29,
  nome: "Bom-bocado",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#bom_bocado",
    "#sobremesa",
    "#coco",
    "#forno"
  ],

  porções: {
    pessoas: "",
    unidades: "12 a 16 unidades"
  },

  tempo_preparo: {
    preparo: "15 minutos",
    forno: "30 minutos",
    descanso: "10 minutos",
    total: "55 minutos"
  },

  ingredientes: {
    principal: [
      "1 lata de leite condensado",
      "2 ovos",
      "2 colheres de sopa de manteiga derretida",
      "1 xícara de chá de coco ralado",
      "1 colher de sopa de farinha de trigo",
      "1 colher de chá de fermento em pó"
    ]
  },

  modo_preparo: {
    preparo: [
      "Em uma tigela, misture o leite condensado, os ovos e a manteiga derretida.",
      "Acrescente o coco ralado e a farinha de trigo, misturando bem.",
      "Por último, adicione o fermento em pó e misture delicadamente."
    ],
    montagem: [
      "Unte forminhas individuais com manteiga.",
      "Despeje a massa nas forminhas, sem encher completamente."
    ],
    assamento: [
      "Leve ao forno preaquecido a 180 °C por cerca de 30 minutos, até dourar levemente."
    ]
  },

  finalização: [
    "Retire do forno.",
    "Deixe amornar antes de desenformar.",
    "Sirva em temperatura ambiente."
  ]
},
// === Receita 30: Rocambole Louco ===
{
  id: 30,
  nome: "Rocambole Louco",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#rocambole",
    "#bolo",
    "#sobremesa"
  ],

  porções: {
    pessoas: "",
    unidades: "1 rocambole"
  },

  tempo_preparo: {
    preparo: "20 minutos",
    forno: "30 minutos",
    total: "50 minutos"
  },

  ingredientes: {
    massa: [
      "6 ovos",
      "12 colheres de açúcar",
      "6 colheres de queijo ralado",
      "1 xícara de calda de laranja"
    ],
    substituição: [
      "A calda de laranja pode ser substituída por 2 colheres de sopa de limão"
    ]
  },

  modo_preparo: {
    'preparo massa': [
      "Bata as claras em neve.",
      "Acrescente as gemas, uma por uma, batendo sempre.",
      "Junte o açúcar, o queijo ralado e a calda de laranja.",
      "Misture bem até obter uma massa homogênea."
    ],
    assamento: [
      "Despeje a massa em forma própria para rocambole, untada.",
      "Leve ao forno preaquecido a 180 °C até assar."
    ],
    montagem: [
      "Retire do forno.",
      "Desenforme ainda morno e enrole cuidadosamente."
    ]
  }
},
// === Receita 31: Glacê de Chocolate ===
{
  id: 31,
  nome: "Glacê de Chocolate",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#glace",
    "#cobertura",
    "#chocolate"
  ],

  porções: {
    pessoas: "",
    unidades: "para 1 rocambole"
  },

  tempo_preparo: {
    preparo: "5 minutos",
    total: "5 minutos"
  },

  ingredientes: {
    principal: [
      "1 pacotinho de margarina",
      "4 colheres de sobremesa de rum",
      "1 colher de Nescau",
      "1 colher de açúcar"
    ]
  },

  modo_preparo: {
    preparo: [
      "Misture bem todos os ingredientes até formar um creme homogêneo."
    ],
    finalização: [
      "Cubra o rocambole já frio com o glacê."
    ]
  }
},
// === Receita 32: Mais de Banana ===
{
  id: 32,
  nome: "Mais de Banana",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#banana",
    "#bolo",
    "#sobremesa",
    "#forno"
  ],

  porções: {
    pessoas: "",
    unidades: "1 bolo grande"
  },

  tempo_preparo: {
    preparo: "30 minutos",
    forno: "40 a 50 minutos",
    total: "1 hora e 20 minutos"
  },

  ingredientes: {
    base: [
      "12 bananas-prata",
      "Açúcar a gosto",
      "Manteiga a gosto para untar"
    ],
    massa: [
      "1 xícara de manteiga",
      "1 xícara de açúcar",
      "1 xícara de farinha de trigo",
      "5 ovos",
      "1 colher de chá de fermento em pó (Pó Royal)",
      "1 pitada de sal",
      "1 colher de chá de raspas de limão"
    ],
    substituições: [
      "A banana pode ser substituída por abacaxi ou maçã"
    ]
  },

  modo_preparo: {
   'preparo forma': [
      "Unte uma forma redonda grande com bastante manteiga e açúcar.",
      "Arrume no fundo uma camada de bananas cortadas em rodelas.",
      "Polvilhe açúcar.",
      "Faça outra camada de bananas e açúcar.",
      "Repita as camadas até acabarem as bananas."
    ],
    'preparo massa': [
      "Bata a manteiga com o açúcar, as raspas de limão e o sal.",
      "Junte as gemas e bata novamente até obter um creme claro e esbranquiçado.",
      "Adicione a farinha de trigo e o fermento em pó.",
      "Por último, incorpore delicadamente as claras em neve."
    ],
    montagem: [
      "Quando a massa começar a formar bolhas, espalhe-a cuidadosamente sobre as bananas."
    ],
    assamento: [
      "Leve ao forno moderado até assar completamente."
    ],
    finalização: [
      "Retire do forno.",
      "Desenforme ainda morno."
    ]
  }
},
// === Receita 33: Manjar de Ameixa ===
{
  id: 33,
  nome: "Manjar de Ameixa",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#manjar",
    "#ameixa",
    "#sobremesa",
    "#geladeira"
  ],

  porções: {
    pessoas: "",
    unidades: "1 forma média"
  },

  tempo_preparo: {
    preparo: "10 minutos",
    geladeira: "4 horas",
    total: "4 horas e 10 minutos"
  },

  ingredientes: {
    principal: [
      "1/2 litro de leite",
      "200 g de ameixa sem semente",
      "1 lata de creme de leite",
      "8 colheres de açúcar",
      "3 folhas de gelatina branca"
    ]
  },

  modo_preparo: {
    preparo: [
      "Dissolva as folhas de gelatina no leite quente.",
      "Coloque todos os ingredientes no liquidificador.",
      "Bata até obter uma mistura homogênea."
    ],
    geladeira: [
      "Despeje em uma forma ou recipiente.",
      "Leve à geladeira até firmar."
    ]
  }
},
// === Receita 34: Pavê de Morango ===
{
  id: 34,
  nome: "Pavê de Morango",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#pave",
    "#morango",
    "#sobremesa",
    "#geladeira"
  ],

  forma: "Forma lisa forrada com papel-manteiga",

  porções: {
    pessoas: "8 a 10",
    unidades: ""
  },

  tempo_preparo: {
    preparo: "20 minutos",
    geladeira: "4 horas",
    total: "4 horas e 20 minutos"
  },

  ingredientes: {
    principal: [
      "1/2 kg de morangos",
      "1/2 kg de biscoito tipo palito francês",
      "2 colheres de sopa de açúcar",
      "2 colheres de sopa de conhaque",
      "1/2 litro de creme de leite sem soro (2 latas)"
    ]
  },

  modo_preparo: {
    preparo: [
      "Lave os morangos e retire os talos.",
      "Reserve alguns morangos inteiros para decoração.",
      "Amasse os morangos com um garfo.",
      "Misture os morangos amassados com o açúcar, o conhaque e 1/3 do creme de leite."
    ],
    montagem: [
      "Forre uma forma lisa com papel-manteiga.",
      "Faça camadas alternadas de biscoito e creme.",
      "Finalize com uma camada de biscoitos.",
      "Comprima levemente."
    ],
    geladeira: [
      "Cubra e leve à geladeira com um peso por cima.",
      "Após gelar, desenforme.",
      "Cubra com o restante do creme de leite.",
      "Decore com os morangos reservados."
    ]
  },

  finalização: [
    "Mantenha refrigerado até a hora de servir."
  ]
}
,
// === Receita 35: Torta de Coco ===
{
  id: 35,
  nome: "Torta de Coco",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#torta",
    "#coco",
    "#sobremesa",
    "#forno"
  ],

  forma: "Forma redonda untada",

  porções: {
    pessoas: "8 a 10",
    unidades: ""
  },

  tempo_preparo: {
    preparo: "30 minutos",
    descanso: "15 minutos",
    forno: "40 minutos",
    total: "1 hora e 25 minutos"
  },

  ingredientes: {
    recheio: [
      "1 coco grande ralado",
      "1 lata de leite condensado (Leite Moça)"
    ],
    massa: [
      "2 xícaras de farinha de trigo",
      "3 colheres de chá de fermento em pó (Pó Royal)",
      "1/2 colher de sopa de açúcar",
      "125 g de manteiga",
      "3 gemas",
      "2 colheres de sopa de leite"
    ],
    creme: [
      "3 gemas",
      "6 colheres de sopa de açúcar",
      "3 colheres de sopa de farinha de trigo",
      "3 xícaras de leite",
      "1 pitada de sal"
    ]
  },

  modo_preparo: {
    'preparo massa': [
      "Misture a farinha de trigo, o fermento e o açúcar.",
      "Acrescente a manteiga, o leite e as gemas.",
      "Misture com a ponta dos dedos, sem sovar a massa.",
      "Forme uma massa homogênea.",
      "Deixe descansar por 15 minutos."
    ],
    'preparo creme': [
      "Bata as gemas com o açúcar.",
      "Acrescente a farinha de trigo e misture bem.",
      "Adicione o leite e o sal.",
      "Leve ao fogo, mexendo sempre, até engrossar.",
      "Retire do fogo e reserve."
    ],
    montagem: [
      "Forre o fundo e as laterais da forma com a massa.",
      "Espalhe o creme sobre a massa.",
      "Distribua o coco ralado por cima.",
      "Regue com o leite condensado."
    ]
  },

  finalização: [
    "Leve ao forno preaquecido a 180 °C até dourar.",
    "Retire do forno e deixe esfriar.",
    "Decore com frutas a gosto."
  ]
},
// === Receita 36: Gelatina do Rei Albert ===
{
  id: 36,
  nome: "Gelatina do Rei Albert",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#gelatina",
    "#sobremesa",
    "#abacaxi",
    "#fria"
  ],

  forma: "Forma lisa levemente untada",

  porções: {
    pessoas: "8 a 10",
    unidades: ""
  },

  tempo_preparo: {
    preparo: "25 minutos",
    geladeira: "4 horas",
    total: "4 horas e 25 minutos"
  },

  ingredientes: {
    base: [
      "6 claras",
      "6 colheres de sopa de açúcar",
      "10 folhas de gelatina vermelha",
      "1 xícara de água"
    ],
    recheio: [
      "1 lata de abacaxi em pedaços"
    ],
    cobertura: [
      "6 gemas",
      "6 colheres de sopa de açúcar"
    ]
  },

  modo_preparo: {
    preparo_base: [
      "Bata as claras em neve.",
      "Junte o açúcar e bata como para suspiro.",
      "Derreta as folhas de gelatina na água.",
      "Quando ferver, coe a gelatina em um pano.",
      "Misture a gelatina coada ao suspiro.",
      "Acrescente o abacaxi picado e misture bem.",
      "Coloque em uma forma e leve à geladeira até firmar."
    ],
    preparo_cobertura: [
      "Bata as gemas com o açúcar até quase o ponto de gemada.",
      "Despeje essa mistura sobre a gelatina já firme."
    ]
  },

  finalização: [
    "Leve novamente à geladeira para firmar.",
    "Desenforme.",
    "Decore com frutas a gosto."
  ]
},
// === Receita 37: Papos de Anjo ===
{
  id: 37,
  nome: "Papos de Anjo",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#sobremesa",
    "#tradicional",
    "#doces_conventuais",
    "#forno"
  ],

  forma: "Forminhas individuais untadas com manteiga",

  porções: {
    pessoas: "10 a 12",
    unidades: "aproximadamente 20 unidades"
  },

  tempo_preparo: {
    preparo: "30 minutos",
    forno: "20 minutos",
    calda: "15 minutos",
    total: "1 hora e 5 minutos"
  },

  ingredientes: {
    massa: [
      "6 ovos (somente as gemas)"
    ],
    calda: [
      "1 kg de açúcar",
      "1 litro de água",
      "1/2 fava de baunilha"
    ]
  },

  modo_preparo: {
    preparo_massa: [
      "Bata bem as gemas até ficarem claras e com o dobro do volume.",
      "Coloque pequenas porções da mistura em forminhas untadas com manteiga.",
      "Leve ao forno até crescerem e dourarem."
    ],
    preparo_calda: [
      "Leve ao fogo o açúcar, a água e a fava de baunilha.",
      "Deixe ferver até formar uma calda clara e transparente."
    ],
    finalizacao: [
      "Retire os papos de anjo das forminhas.",
      "Coloque-os diretamente na calda ainda quente.",
      "Deixe ferver até ficarem bem transparentes."
    ]
  }
},
// === Receita 38 (versão simplificada): Torta Folhada de Nozes ===
{
  id: 38,
  nome: "Torta Folhada de Nozes",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#torta",
    "#nozes",
    "#sobremesa",
    "#forno"
  ],

  forma: "Forma redonda untada com água fria",

  porções: {
    pessoas: "8 a 10",
    unidades: ""
  },

  tempo_preparo: {
    preparo: "40 minutos",
    descanso: "10 minutos",
    forno: "40 minutos",
    total: "1 hora e 30 minutos"
  },

  ingredientes: {
    massa: [
      "1 1/2 xícara de farinha de trigo",
      "6 colheres de sopa de manteiga",
      "1 colher de chá de sal",
      "1/2 xícara de água"
    ],
    recheio: [
      "4 xícaras de nozes moídas (aproximadamente 100 g)",
      "1 colher de sopa de amido de milho (Maizena)",
      "1 colher de chá de baunilha",
      "1/3 de xícara de açúcar",
      "Gemas (das claras utilizadas)",
      "2 claras",
      "Manteiga (para bater o recheio)"
    ]
  },

  modo_preparo: {
    preparo_massa: [
      "Peneire a farinha sobre uma superfície limpa.",
      "Faça uma cova no centro e coloque o sal e a água.",
      "Misture até formar uma massa lisa, que não grude nas mãos.",
      "Abra a massa com um rolo.",
      "Espalhe parte da manteiga sobre a massa.",
      "Dobre a massa em três partes.",
      "Abra novamente e repita esse processo até usar toda a manteiga (4 dobras no total).",
      "Reserve a massa."
    ],
    preparo_recheio: [
      "Bata a manteiga com 1/3 de xícara de açúcar até formar um creme.",
      "Junte as gemas, uma a uma.",
      "Acrescente as nozes moídas, o amido de milho e a baunilha.",
      "Bata as claras em neve.",
      "Misture as claras com o restante do açúcar.",
      "Incorpore delicadamente ao recheio."
    ],
    montagem: [
      "Divida a massa em três partes iguais.",
      "Reserve uma parte para fazer o gradeado.",
      "Abra as outras duas partes.",
      "Forre o fundo e as laterais da forma.",
      "Coloque o recheio.",
      "Leve ao forno por 10 minutos.",
      "Retire do forno.",
      "Faça um gradeado com a massa reservada."
    ]
  },

  finalizacao: [
    "Leve novamente ao forno por mais 30 minutos.",
    "Retire do forno e deixe esfriar antes de desenformar."
  ]
},
// === Receita 39: Taça Havaí ===
{
  id: 39,
  nome: "Taça Havaí",
  categoria: "Doce",
  tags: [
    "#doce", // 🔹 TAG OBRIGATÓRIA
    "#sobremesa",
    "#sorvete",
    "#frutas",
    "#gelada"
  ],

  forma: "Taças individuais",

  porções: {
    pessoas: "5",
    unidades: "5 taças"
  },

  tempo_preparo: {
    preparo: "30 minutos",
    geladeira: "2 horas",
    total: "2 horas e 30 minutos"
  },

  ingredientes: {
    creme: [
      "1 lata de leite condensado",
      "1 lata de água",
      "2 gemas",
      "1 colher de sopa de farinha de trigo",
      "1 colher de sopa de açúcar",
      "1 colher de sopa de suco de limão"
    ],
    frutas: [
      "3 abacaxis",
      "1 lata de morango picado",
      "Cerejas em calda"
    ],
    montagem: [
      "Sorvete de morango"
    ]
  },

  modo_preparo: {
    preparo_creme: [
      "Corte os abacaxis ao meio no sentido do comprimento.",
      "Descasque uma metade e reserve as outras metades.",
      "Bata no liquidificador a metade descascada do abacaxi.",
      "Utilize aproximadamente 1 xícara e meia do suco obtido.",
      "Misture o suco do abacaxi com as gemas, a farinha e o açúcar.",
      "Leve ao fogo brando, mexendo sem parar, até obter um creme.",
      "Retire do fogo e deixe esfriar.",
      "Leve à geladeira para gelar."
    ],
    preparo_frutas: [
      "Retire o centro duro do abacaxi reservado.",
      "Retire a polpa e pique em pedacinhos.",
      "Reserve."
    ],
    montagem: [
      "Coloque um pouco do creme no fundo da taça.",
      "Sobre o creme, coloque três bolas de sorvete de morango.",
      "Decore com pedaços de abacaxi, cerejas em calda ou morangos frescos."
    ]
  }
}




















]